## 概要
* Rails on Rails による Web バックエンド開発者（フリーランス）です
* 業務ロジックの実装と、RSpec を用いたテストコード実装が好きで、これらを得意分野としています
* 要件がふわっとしていても、詳細設計（＆レビュー） 〜 実装（＆テスト）をバイネームで遂行することができます
* 自分の手を止めてでもコードレビューする派です
  - Pull Request の滞留時間が、チーム全体の生産性や効率、及びレビューイーのモチベーションに大きな影響を及ぼすと考えているからです
  - メンバーがどんなコードを書くのか？”自分だったらこう書く”との違いは何か？の好奇心が根底にあるからです
  - 自分の進捗が遅れると元も子もないので、そこはいー感じに振る舞います
  - コードレビューしない人は、スキルアップの機会を逃していると共に、解り易い Pull Request が書けないと確信しています（故にレビュー文化を重視します）
